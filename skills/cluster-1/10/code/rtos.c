...
static void test_alpha_display() {
    ...
    while (1) {
      if (direction == 0) {
        displaybuffer[0] = bin[(int)'U'];   // T.
        displaybuffer[1] = bin[(int)'P'];  // D.
        displaybuffer[2] = bin[(int)'-'];  // C.
        displaybuffer[3] = bin[(int)'-'];  // L.
      }
      else {
        displaybuffer[0] = bin[(int)'D'] ;  // T.
        displaybuffer[1] = bin[(int)'O'];  // D.
        displaybuffer[2] = bin[(int)'W'];  // C.
        displaybuffer[3] = bin[(int)'N'];  // L.
      }
...
static void check_button() {
  ...
  while(1) {
      ...
      button_pressed = gpio_get_level(BUTTON_GPIO);
      //printf("Button val: %d\n", button_pressed);
...
static void decimal_to_binary()
{
  ...
  while(1)
  {
    ...
    gpio_set_level(BLINK_GPIO,gp1);
    gpio_set_level(BLINK_GPIO2,gp2);
    gpio_set_level(BLINK_GPIO3,gp3);
    gpio_set_level(BLINK_GPIO4,gp4);
